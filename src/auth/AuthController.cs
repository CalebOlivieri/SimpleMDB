...
public class AuthController
{
    ...
    public async Task LandingPageGet(HttpListenerRequest req, HttpListenerResponse res, Hashtable options)
    {
        ...
        string message = req.QueryString["message"] ?? ""; // Or req.QueryString["message"] ?? ""; if you want to pass messages to the landing page
    ...
    public async Task RegisterGet(HttpListenerRequest req, HttpListenerResponse res, Hashtable options)
    {
        ...
        <input id=""username"" name=""username"" type=""text"" placeholder=""Username"" vale=""{username}"">
    ...
    public async Task RegisterPost(HttpListenerRequest req, HttpListenerResponse res, Hashtable options)
    {
        ...
        {
            HttpUtils.AddOptions(options, "redirect", "message", "Password do not match");
    ...
    {
        string token = req.Headers["Authorizartion"]?.Substring(7) ?? req.Cookies["token"]?.Value ?? "";
